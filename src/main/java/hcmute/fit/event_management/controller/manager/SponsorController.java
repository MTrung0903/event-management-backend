...
public class SponsorController {
    ...
    public ResponseEntity<?> createSponsorByEventId(@RequestParam("eid") int eid ,@RequestBody SponsorEventDTO sponsorEventDTO) {
        ...
        sponsorEvent.setEvent(eventService.findById(eid).orElse(new Event()));
        sponsorEventService.save(sponsorEvent);
        Response response = new Response(200,"",null);
        return new ResponseEntity<>(response,HttpStatus.OK);
    }
    @PutMapping("/myevent/sponsor")
    public ResponseEntity<?> updateSponsorByEventId(@RequestParam("eid") int eid, @RequestBody SponsorEventDTO sponsorEventDTO) {
        Sponsor sponsor = new Sponsor();
        BeanUtils.copyProperties(sponsorEventDTO, sponsor);
        sponsor = sponsorService.save(sponsor);
        SponsorEvent sponsorEvent = new SponsorEvent();
